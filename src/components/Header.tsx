  ...
  return (
    <>
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {navLinks.map((link) => (
            <button
              ...
              className={`relative px-2.5 xl:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold tracking-wider rounded-lg transition-[color,background-color,box-shadow] duration-300 whitespace-nowrap ${
                activePage === link.id
            ...
            >
              {link.name}
              {activePage === link.id && (
                <span className="absolute bottom-0 left-3 right-3 h-[2px] rounded-full bg-gradient-to-r from-purple-500 to-cyan-500 shadow-[0_0_8px_rgba(124,58,237,0.5)]" />
              )}
            </button>
          ))}
        </nav>
      ...
      {mounted && mobileMenuOpen && (
        <div className="fixed inset-0 z-30 bg-[#050816]/98 backdrop-blur-2xl lg:hidden flex flex-col justify-center items-center px-4 overflow-hidden">
          ...
          <nav className="flex flex-col gap-2 sm:gap-4 text-center w-full max-w-xs">
            {navLinks.map((link, idx) => (
              <button
                key={link.id}
                onClick={() => handleNav(link.id)}
                style={{ animationDelay: `${idx * 0.05}s` }}
                className={`relative text-lg sm:text-2xl md:text-3xl font-bold tracking-widest uppercase py-3 sm:py-2 rounded-xl touch-manipulation overflow-hidden animate-[fade-in-up_0.4s_ease-out_both] ${
                  activePage === link.id
              ...
              >
                {link.name}
                {activePage === link.id && (
                  <span className="absolute -bottom-0.5 left-1/2 -translate-x-1/2 w-8 h-[2px] rounded-full bg-gradient-to-r from-purple-500 to-cyan-500" />
                )}
              </button>
            ))}
            <div className="flex items-center justify-center gap-5 sm:gap-6 mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-white/10 animate-[fade-in-up_0.4s_ease-out_0.35s_both]">
              {[
